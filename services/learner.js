...
async function scanForumChannel(forumChannel, guildId, onProgress = null) {
    ...
    if (textThreads.length > 0) {
        const system = `You are extracting structured knowledge from a Discord forum channel in a sim-racing league.
...
async function buildChannelDirectory(guild, guildId, onProgress = null) {
    ...
    const system = `You are building a channel directory for ${guild.name}, a sim-racing Discord server.
